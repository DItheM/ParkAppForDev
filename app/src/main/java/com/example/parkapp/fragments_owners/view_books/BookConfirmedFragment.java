...
public class BookConfirmedFragment extends Fragment {
    ...
    public BookConfirmedFragment(String mode) {
        this.mode = mode;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_book_confirmed, container, false);
        ...
        if (DatabaseHelper.userId() != null) {
            if (mode.equals("owner")) {
                mDatabase.child("Owners").child(DatabaseHelper.userId()).child("userRequests").child("book").child("confirmed").addValueEventListener(new ValueEventListener() {
                    @Override
                ...
                mDatabase.child("Organizations").child("data").child(DatabaseHelper.userId()).child("book").child("confirmed").addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        message.setVisibility(View.GONE);
                        list.clear();
        ...
        adapter = new RecycleViewAdapterBookRequests(getContext(), list, mode, 1);
